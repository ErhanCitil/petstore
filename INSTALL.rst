...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/petstore.git
       $ cd petstore
...
``src/petstore/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test petstore
...
* ``DB_NAME``: name of the database for the project. Defaults to ``petstore``.
* ``DB_USER``: username to connect to the database with. Defaults to ``petstore``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``petstore``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting petstore_db_1 ... done
       Starting petstore_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it petstore_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it petstore_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/petstore/
...
environment variables. See ``src/petstore/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker exec -it petstore /app/src/manage.py createsuperuser
